...
export default function BudgetPlanEditor({ month, year, onClose, authFetch }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 backdrop-blur-sm transition-colors" style={{ background: t.overlay }}>
      <div className="w-full max-w-lg h-[90vh] rounded-[2rem] shadow-2xl flex flex-col relative overflow-hidden" style={{ background: t.card, border: `1px solid ${t.border}`, color: t.text }}>
        
        <div className="px-5 py-4 md:px-6 md:py-5 border-b flex-shrink-0 flex items-center justify-between" style={{ borderColor: t.border }}>
          <div>
            <h3 className="text-lg md:text-xl font-bold flex items-center gap-2"><Target size={20} color={t.accent} /> Budget Plan</h3>
            <p className="text-[10px] md:text-xs font-semibold mt-1" style={{ color: t.textMuted }}>Set planned amounts & dates</p>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-4 md:p-6 space-y-6 md:space-y-8">
          <div className="p-4 md:p-5 rounded-2xl border" style={{ background: t.inputBg, borderColor: t.border }}>
            <div className="flex justify-between items-center mb-3">
              <label className="text-xs md:text-sm font-bold flex items-center gap-2"><Wallet size={16} color={t.accent} /> Start Balance</label>
            </div>
            ...
            return (
              <div key={sec}>
                <div className="flex justify-between items-end mb-4 border-b pb-2" style={{ borderColor: isIncome ? t.green : t.border }}>
                  <h4 className="font-bold text-xs md:text-sm" style={{ color: isIncome ? t.green : t.accent }}>{isIncome ? "↑" : "↓"} {sec}</h4>
                  <span className="text-[10px] md:text-xs font-bold" style={{ color: t.textMuted }}>Total: ₵{sectionTotal(subs).toFixed(2)}</span>
                </div>
                <div className="space-y-4">
                  {subs.map(sub => (
                    <div key={sub} className="flex items-center gap-2 md:gap-3">
                      <div className="flex-1 min-w-0">
                        <label className="text-[10px] md:text-xs font-bold block mb-1 truncate" style={{ color: t.text }}>{sub}</label>
                        <input type="number" placeholder="0.00" value={plans[sub]?.budget_amount || ""} onChange={e => handleChange(sub, "budget_amount", e.target.value)} className="w-full px-3 py-2 rounded-lg outline-none text-xs md:text-sm font-semibold transition-colors" style={{ background: t.inputBg, border: `1px solid ${t.border}` }} />
                      </div>
                      <div className="w-[100px] md:w-1/3 flex-shrink-0">
                        <label className="text-[9px] md:text-[10px] font-bold uppercase tracking-wider block mb-1" style={{ color: t.textMuted }}>Date</label>
                        <input type="date" value={plans[sub]?.expected_date || ""} onChange={e => handleChange(sub, "expected_date", e.target.value)} className="w-full px-1 md:px-2 py-2 rounded-lg outline-none text-[10px] md:text-xs font-medium transition-colors" style={{ background: t.inputBg, border: `1px solid ${t.border}`, color: t.textMuted }} />
                      </div>
                    </div>
                  ))}
                </div>
